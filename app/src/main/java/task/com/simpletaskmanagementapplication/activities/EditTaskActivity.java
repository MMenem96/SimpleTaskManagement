...
public class EditTaskActivity extends AppCompatActivity implements DatePickerDialog.OnDateSetListener {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.menu_add_task) {
            if (dataIsValid()) {
                Task newTask = new Task(task.getId(), etTaskTitle.getText().toString().trim(), etTaskDetails.getText().toString().trim(), currentDate, priority, 0, task.getTaskTimeStamp());
                long rowInserted = dbHelper.updateTask(newTask);
                if (rowInserted == 1) {
    ...
    private boolean dataIsValid() {
        ...
        if (TextUtils.isEmpty(etTaskDetails.getText().toString().trim())) {
            etTaskDetails.setError(getString(R.string.empty_field));
            return false;
        }
        return true;
    }
